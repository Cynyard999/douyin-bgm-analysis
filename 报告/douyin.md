...
- [3. 研究方法](#3-研究方法)
    - [3.1 数据获取](#31-数据获取)
        ...
        - [3.1.2 音乐格式转换与时长处理](#312-音乐格式转换与时长处理)
    - [3.2 数据处理](#32-数据处理)
    - [3.3 特征提取](#33-特征提取)
        - [3.3.1 波形统计特征提取](#331-波形统计特征提取)
        - [3.3.2 筛选特征值的最佳子集](#332-筛选特征值的最佳子集)
        - [3.3.3 归一化和降维处理](#333-归一化和降维处理)
    - [3.4 音乐分类](#34-音乐分类)
        - [3.4.1 K-Means聚类](#341-k-means聚类)
            - [3.4.1.1 传统K-Means算法](#3411-传统k-means算法)
            - [3.4.1.2 MiniBatchKMeans算法](#3412-minibatchkmeans算法)
            - [3.4.1.3 选择K值](#3413-选择k值)
            - [3.4.1.4 聚类结果](#3414-聚类结果)
        - [音乐音调变化图(这部分貌似写分析方法的时候可以用上 所以我没删 然后midi文件生成应该是用来获取和弦还是啥的吧 不知道放在哪块我就没动)](#音乐音调变化图这部分貌似写分析方法的时候可以用上-所以我没删-然后midi文件生成应该是用来获取和弦还是啥的吧-不知道放在哪块我就没动)
        - [midi文件生成](#midi文件生成)
    - [研究方法](#研究方法)
        - [音乐流派分类](#音乐流派分类)
    - [4. 案例分析](#4-案例分析)
...
#### 3.4.1.4 聚类结果
最后，我们可以根据聚类结果生成音乐信息分类情况，为我们后续的热门音乐流派分析提供测试集：
